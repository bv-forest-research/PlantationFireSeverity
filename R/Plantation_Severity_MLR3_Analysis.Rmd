...
```{r}
#dat_names <- list.files(path="./Inputs/Vectors/", pattern = "cat.gpkg", full.names = TRUE) #Key and Benson classification
dat_names <- list.files(path="./Inputs/Vectors/", pattern = "cal.gpkg", full.names = TRUE) #calibrated thresholds from CBI plots
...
for(i in 1:length(task_names)){
  ...
  task <- as_task_classif_st(dat, target = "dNBR_cal", 
                             id = task_names[i])
...
fwrite(RF_dat, "./Outputs/Tables/RF_ClassificationError_cal2.csv")
...
fwrite(TrainImp_dt, "./Outputs/Tables/RF_PartialScores_cal2.csv")
...
```{r}
# FIGURES -------------------------------------------------------------------------------------------
TrainImp_dt <- fread("./Outputs/Tables/RF_PartialScores_cal2.csv")
...
dat_csv_dt[, dNBR_cal := cut(dNBR*1000, breaks = m, labels = c("1","2","3","4"),
...
ggplot(dat_csv_dt, aes(x=PlantAge, y=dNBRReSamp))+
  ...
  ylab("Burn severity (dNBR)")+
  theme_minimal()+
  facet_wrap("FireID", nrow = 3, ncol = 2)+
  theme(strip.text.x = element_text(face="bold"),text=element_text(size=15))
ggsave(filename = "Fig6.jpg",path = "./Outputs/Figures/", device='jpeg', dpi=500, bg="white")
...
ggplot(dat_csv_dt, aes(x=CROWN_CLOS, y=dNBRReSamp))+
  geom_jitter(aes(colour = as.factor(dNBR_cal)))+
  scale_color_manual(name="Severity", labels=c("Unburned","Low","Moderate","High"),
                     values=c("darkgreen","yellow","orange","red"))+
  ...
  facet_wrap("FireID")+
  theme(strip.text.x = element_text(face="bold"),text=element_text(size=15))
ggsave(filename = "FigS9.jpg",path = "./Outputs/Figures/", device='jpeg', dpi=1000, bg="white")
...
ggplot(dat_csv_dt, aes(x=BASAL_AREA, y=dNBRReSamp))+
  geom_point(aes(colour = as.factor(dNBR_cal)))+
  scale_color_manual(name="Severity", labels=c("Unburned","Low","Moderate","High"),
                     values=c("darkgreen","yellow","orange","red"))+
  ...
  facet_wrap("FireID")+
  theme(strip.text.x = element_text(face="bold"),text=element_text(size=15))
ggsave(filename = "FigS10.jpg",path = "./Outputs/Figures/", device='jpeg', dpi=1000, bg="white")
...
ggplot(dat_csv_dt, aes(x=maxT, y=dNBRReSamp))+
  ...
  scale_color_manual(name="Severity", labels=c("Unburned","Low","Moderate","High"),
                     values=c("darkgreen","yellow","orange","red"))+
  geom_smooth(aes(x=maxT, y=dNBRReSamp), method="lm", formula= y ~ s(x),
              linewidth=1.5, colour="black")+
  ...
  facet_wrap("FireID")+
  theme(strip.text.x = element_text(face="bold"),text=element_text(size=15))
ggsave(filename = "Fig11.jpg",path = "./Outputs/Figures/", device='jpeg', dpi=1000, bg="white")
