...
##THIS IS ALANA CLASON'S MODIFICATIONS TO CODE PROVIDED BY SEAN PARKES DEC 12, 2020.
...
hotspots <- do.call(rbind, hotspots.list)

#fire.list <- unique(subset(fire.perims, Year == year)$Fire_ID)
fire.list <- unique(subset(fire.perims, FIRE_YEAR == year)$FIRE_NUMBE)

for (xx in 1:length(fire.list)) {
		
	fire <- fire.list[[xx]]

	fire.shp <- subset(fire.perims, FIRE_NUMBE == fire)
...
for (xx in 1:length(fire.list)) {
	...
	if (dir.exists(paste0('./Inputs/Rasters/DOB/', fire)) == T) {
		...
		if (nrow(fire.hotspots) >= 10) {
			...
			xy$extract <- extract(fire.perim.raster, xy)
			xy <- na.omit(xy)
			xy <- xy[,c(1,2)]
...
for (xx in 1:length(fire.list)) {
	...
	if (file.exists(paste0('./Inputs/Rasters/DOB/', fire, '/dob.tmp.tif')) & dir.exists(paste0('./Inputs/Rasters/DOB/', fire)) == T) {
		...
		xy <- na.omit(xy)
		xy$DOB <- extract(modeled.dob.raster, xy[,c(1,2)])
		xy$ID <- row.names(xy)
